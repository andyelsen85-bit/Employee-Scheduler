...
export function generatePlanning(params: {
  ...
  // IMPORTANT: do NOT restrict to thisMonthPlanningDaySet here.  The user can manually lock a JL
  // on an overflow day (e.g. Jul 1 while viewing July when July starts mid-week).  Those dates are
  // outside thisMonthPlanningDaySet (their ISO week belongs to the previous month) but they are
  // still in July's lockedEntries and count against July's jlDays budget.
  // distributeJlDays only draws dates from thisMonthPlanningDays, so recording an overflow date in
  // lockedJlDatesByEmp is harmless — it will never be chosen — but the count reduction IS critical.
  ...
  // Also fold in JL from the previous month's overflow that have NOT been overridden by a locked
  // entry in the current month (e.g. a JL auto-generated by June's planner for Jul 1 that the user
  // has not touched).  These are already excluded from lockedEntries (they live in June's plan),
  // yet they are visible in July's view and consume one of the employee's JL days for this month.
